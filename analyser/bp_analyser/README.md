...
- abandons the cycle in hand when `POST /api/cancel` leaves one, at the next
  point between two steps where stopping is safe. A cancellation arriving
  while nothing is running withdraws the work that would have started the
  next cycle instead, so cancelling straight after sharing does not merely
  postpone the run;
...
into a cancel button, which stops the app waiting and sends `POST /api/cancel`
to the analyser so the run stops too; the ring is pressable without a hover as
well, since a touch screen never reports one. The two halves are independent:
the app stops immediately, and the analyser stops at its next checkpoint. If
the analyser cannot be reached — the API is off, or bound to a loopback
address on another machine — the app says so and stops waiting anyway, leaving
the run to finish unwatched.

Readings already shared stay shared. Withdrawing them is a separate decision,
made in the file browser, and undoing it silently would be a surprise.

The app finds the analyser at `Analyser.apiBaseUrl` in
`lib/constants/analyser.dart`, `http://localhost:8088` by default. A
deployment elsewhere overrides it at build time:

```bash
flutter run \
  --dart-define=HEALTHPOD_ANALYSER_API=https://analyser.example.org \
  --dart-define=HEALTHPOD_ANALYSER_API_TOKEN="$TOKEN"
```

Setting `HEALTHPOD_ANALYSER_API` to the empty string switches the call off,
and cancelling then only stops the app waiting.
