...
ollama_client = OllamaClient(
    ...
    timeout=int(os.getenv("OLLAMA_TIMEOUT", "300"))  # 300 seconds (5 minutes)
)
...
async def analyse_pdf(
    ...
    logger.info(
        "Consider using /analyse/text endpoint - Flutter extracts text locally")
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to analyse PDF content: {str(e)}"
            )
...
async def analyse_text(request: TextAnalysisRequest):
    ...
    """
    logger.info(
        f"Received text analysis request for report: {request.report_name}")
...
async def analyse_text_internal(
    ...
    logger.info(
        f"Model: {ollama_client.model}, Timeout: {ollama_client.timeout}s")
    try:
